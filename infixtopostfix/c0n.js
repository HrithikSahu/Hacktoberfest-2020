...
EX.InfixToPostfix = function (expression) {
  let pfixString = "";
  let infixStack = new EX.LinkedStack();
  ...
  for (let i = 0; i < expression.length; i++) {
    ...
    if (!isNaN(parseInt(c))) {
      ...
      while (
        c != "^" &&
        !infixStack.isStackEmpty() &&
        precedence(c) <= precedence(infixStack.stackTop())
      ) {
...
let in1 = new EX.InfixToPostfix("2+3*4"); //234*+
console.log("Postfix 2+3*4 ==> " + in1.getPostfix());
let in2 = new EX.InfixToPostfix("2*3-4^2"); //23*42^-
console.log("Postfix 2*3-4^2 ==> " + in2.getPostfix());
let in3 = new EX.InfixToPostfix("3+4*5/6"); //345*6/+
console.log("Postfix 3+4*5/6 ==> " + in3.getPostfix());
let in4 = new EX.InfixToPostfix("4+8*6-5/3-2*2+2"); // 486*+53/-22*-2+
